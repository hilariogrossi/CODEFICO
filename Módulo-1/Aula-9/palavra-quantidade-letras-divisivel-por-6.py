...
programa_tamanho_palavra() que é a função principal do programa, solicita ao usuário que digite
uma palavra e chama a função verificar_tamanho_palavra() para verificar o tamanho da palavra
digitada. Dica: Utilize a função len() para obter o número de caracteres da palavra.'''


def verificar_tamanho_palavra(palavra):
    if len(palavra) % 6 == 0:
        print(f'\nA palavra {palavra} é divisíl por 6.\n')
    else:
         print(f'\nA palavra {palavra} NÃO é divisíl por 6.\n')


def programa_tamanho_palavra():
    palavra = input('\nEntre com uma palavra para verificação: ')
