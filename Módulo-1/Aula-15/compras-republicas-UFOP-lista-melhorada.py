...
num_republica = int(input('Entre com o número de repúblicas para as compras: '))

compras_totais = []

for i in range(num_republica):
    num = int(input(f'Entre com a quantidade de itens para a {i + 1}ª república: '))
    soma_republica = 0

    for j in range(num):
        valor = float(input(f'Entre com o valor do {j + 1}° produto: '))
...
for i, valor_compra in enumerate(compras_totais):
    print(f'Valor da compra da {i + 1}ª república: R$ {valor_compra:.2f}')

print(f'Valor da compra total: R$ {soma_total:.2f}')
